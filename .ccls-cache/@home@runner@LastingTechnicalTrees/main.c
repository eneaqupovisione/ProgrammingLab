...
float gravity_idx(float prezzo,int direzione){
    ...
    float y=pow(x,2);
        ...
        if (prezzo>2 && prezzo<=45){
            gamma=0.05*y +61.5*x;}
        if (prezzo>45 && prezzo<=70){
            gamma=0.140*y +53.4*x;}
        if (prezzo>70 && prezzo<=80.5){
            gamma=0.205*y +44.17*x;}
        if (prezzo>80.5 && prezzo<=82.5){
            gamma=0.775*y -45.61*x;}
        if (prezzo>82.5 && prezzo<=88.5){
            gamma=0.454*y +8.04*x;}
        if (prezzo>88.5 && prezzo<=95){
            gamma=0.369*y +17.9*x;}
        if (prezzo>95 && prezzo<=120){
            gamma=0.140*y +66.4*x;}
        if (prezzo>120 && prezzo<=150){
            gamma=0.048*y+88.30*x;}
    float w=log(gamma);
    
    float z=(((w/4.81)*100)-99);
